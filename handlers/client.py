...
async def res_step(message: types.Message, state: FSMContext):
    ...
    if await acc_verify(user_data["email"], user_data["password"]):
        ...
        await message.answer("Для просмотра статистик используйте кнопки 👇", reply_markup=await kb_client(await db.userExsist(message.from_id)))
    else:
        await message.edit_text("❌ Авторизация не пройдена")
        await message.answer("❗ Имя пользователя или пароль введены неверно", reply_markup=await kb_client(await db.userExsist(message.from_id)))
    await state.finish()
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.register_message_handler(
        auth_step, Text(equals='Авторизация'), state="*")
    dp.register_message_handler(
